...
// Entity fields that can be added as form fields
const LEAD_FIELDS = [
...
export function FormBuilderDialog({
  ...
  // Separate text state for options inputs to fix comma bug
  ...
  const handleSave = () => {
    ...
    // Finalize options from text state
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <textarea
                ...
                rows={2}
                className="mt-1 w-full rounded-lg border border-input bg-background px-3 py-2 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-teal-500"
              />
            </div>
          </div>

          {/* Entity Linking */}
          <div className="rounded-xl border border-dashed border-blue-300 bg-blue-50/50 dark:bg-blue-950/20 dark:border-blue-800 p-4 space-y-3">
            <div className="flex items-center gap-2 mb-1">
              <Link2 className="h-4 w-4 text-blue-600" />
              <label className="text-sm font-semibold text-blue-800 dark:text-blue-300">{t("linkEntity")}</label>
            </div>
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors ${
                  linkType === "none"
                    ? "bg-slate-200 border-slate-300 text-slate-800 dark:bg-slate-700 dark:text-slate-200"
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors ${
                  linkType === "lead"
                    ? "bg-indigo-100 border-indigo-300 text-indigo-800 dark:bg-indigo-900 dark:text-indigo-200"
                    : "border-slate-200 text-slate-500 hover:bg-slate-100 dark:border-slate-700"
                }`}
              >
                ...
                {entityFields.map((ef) => {
                  ...
                  return (
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-dashed border-blue-300 text-blue-700 text-xs font-medium hover:bg-blue-50 hover:border-blue-400 transition-colors dark:border-blue-700 dark:text-blue-400 dark:hover:bg-blue-950"
                    >
              ...
              {fieldTypes.map((type) => {
                ...
                return (
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-dashed border-teal-300 text-teal-700 text-xs font-medium hover:bg-teal-50 hover:border-teal-400 transition-colors dark:border-teal-700 dark:text-teal-400 dark:hover:bg-teal-950"
                  >
                    <Icon className="h-3.5 w-3.5" />
          ...
          <div className="space-y-3">
            ...
            {fields.map((field, index) => {
              ...
              return (
                <div
                  key={field.id}
                  className="border rounded-xl p-4 bg-gradient-to-r from-slate-50 to-gray-50 dark:from-slate-900 dark:to-gray-900 space-y-3"
                >
                  <div className="flex items-center gap-2">
                    <GripVertical className="h-4 w-4 text-muted-foreground/50 shrink-0" />
                    <div className="h-7 w-7 rounded-lg bg-teal-100 dark:bg-teal-900 flex items-center justify-center">
                      <FieldIcon className="h-3.5 w-3.5 text-teal-600 dark:text-teal-400" />
                    </div>
                  ...
                  {(field.type === "select" || field.type === "checkbox") && (
                    <div>
                      ...
                      <Input
                        ...
                        onBlur={() => handleOptionsTextBlur(field.id)}
                        className="mt-0.5 h-8 text-sm"
                  ...
                  {/* Advanced Config */}
                  <div className="rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/30 p-2.5 space-y-2.5">
                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">{t("advancedConfig")}</p>

                    {/* Default value — all types except rating */}
                        ...
                        {(field.type === "text" || field.type === "email") && (
                          <div>
                            ...
                            <Input
                              ...
                              onChange={(e) => updateField(field.id, { maxLength: e.target.value ? Number(e.target.value) : undefined })}
                              className="mt-0.5 h-7 text-xs"
                              placeholder="—"
                              min={1}
                            />
                        ...
                        {field.type === "phone" && (
                          <div>
                            ...
                            <select
                              ...
                              onChange={(e) => updateField(field.id, { phoneCountry: e.target.value })}
                              className="mt-0.5 w-full rounded-md border bg-background px-2 py-1 text-xs"
                            >
                          ...
                          <div className="flex items-end pb-0.5">
                            <label className="flex items-center gap-2 cursor-pointer">
                              <input
                                ...
                                onChange={(e) => updateField(field.id, { dateDefaultToday: e.target.checked })}
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <select
                            ...
                            onChange={(e) => updateField(field.id, { numberDecimalPlaces: Number(e.target.value) })}
                            className="mt-0.5 w-full rounded-md border bg-background px-2 py-1 text-xs"
                          >
                        ...
                        <div className="flex items-end pb-0.5">
                          <label className="flex items-center gap-2 cursor-pointer">
                            <input
                              ...
                              onChange={(e) => updateField(field.id, { numberThousandsSeparator: e.target.checked })}
                              className="h-3.5 w-3.5 rounded border-gray-300 text-teal-600 focus:ring-teal-500"
                            />
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div>
                          ...
                          <select
                            ...
                            onChange={(e) => updateField(field.id, { defaultValue: e.target.value ? Number(e.target.value) : undefined })}
                            className="mt-0.5 w-full rounded-md border bg-background px-2 py-1 text-xs"
                          >
                            <option value="">—</option>
                    ...
                    {field.type === "select" && (
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          ...
                          onChange={(e) => updateField(field.id, { selectMultiple: e.target.checked })}
                          className="h-3.5 w-3.5 rounded border-gray-300 text-teal-600 focus:ring-teal-500"
                        />
